...
LABEL org.opencontainers.image.source="https://github.com/barrow1990/reclaimerr-mcp-server" \
...
# Liveness only (process up, HTTP serving) — not Reclaimerr connectivity, so a
# transient Reclaimerr outage doesn't get Dockhand/Docker restarting this
# container in a loop. Use GET /ready separately to check Reclaimerr connectivity.
